...
$(function() {
        ...
        _.each(app.injections, function(_inj) {
            if ((!app.region_picked) || _inj.region_id == app.region_picked.id) {
                ...
                if (injection_count >= start && injection_count < start + limit) {
                    ...
                    var text_class = '';
                    if (_inj.status == 'Hidden') {
                        text_class = 'strike-through';
                    }
                    text += '<tr data-id="' + _inj.id + '" style="" class="' + text_class + '"><td>' + _inj.tracer + '</td><td style="background-color: ' + bg_color +';">' + _region.code + '</td><td>' + _inj.l.toFixed(1) + '</td><td>' + _inj.a.toFixed(1) + '</td><td>' + _inj.h.toFixed(1) + '</td><td>' + _inj.case_id + '</td></tr>';
                }
                injection_count++;
            }
        });
    ...
    var canvas_mouse_move = function(e) {
        ...
        if (nearest && nearest_distance < 160) {
            ...
            app.region_picked = region_picked = _.find(app.regions, {id: nearest.region_id});
        } else {
